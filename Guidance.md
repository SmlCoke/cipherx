...
- [目录](#目录)
- [CipherX CLI 工具开发指南](#cipherx-cli-工具开发指南)
  - [项目概述](#项目概述)
  - [一、工具功能清单](#一工具功能清单)
    - [1.1 核心功能](#11-核心功能)
      - [基础功能](#基础功能)
      - [帮助系统](#帮助系统)
    - [1.2 加密算法实现（第一版）](#12-加密算法实现第一版)
      - [1. 凯撒密码 (Caesar Cipher)](#1-凯撒密码-caesar-cipher)
      - [2. 替换密码 (Substitution Cipher)](#2-替换密码-substitution-cipher)
      - [3. 维吉尼亚密码 (Vigenère Cipher)](#3-维吉尼亚密码-vigenère-cipher)
      - [4. 栅栏密码 (Rail Fence Cipher)](#4-栅栏密码-rail-fence-cipher)
      - [5. Base64 编码](#5-base64-编码)
    - [1.3 后续版本扩展功能（预留设计）](#13-后续版本扩展功能预留设计)
      - [对称加密算法](#对称加密算法)
      - [非对称加密算法](#非对称加密算法)
      - [其他功能](#其他功能)
  - [二、C/C++ 详细开发步骤](#二cc-详细开发步骤)
    - [2.1 开发环境准备](#21-开发环境准备)
      - [Windows 平台（推荐使用 MSYS2）](#windows-平台推荐使用-msys2)
      - [Linux 平台](#linux-平台)
      - [macOS 平台](#macos-平台)
    - [2.1.1 配置 VSCode 开发环境（推荐）](#211-配置-vscode-开发环境推荐)
      - [1. 安装 VSCode](#1-安装-vscode)
      - [2. 安装必要的扩展](#2-安装必要的扩展)
      - [3. 配置 C/C++ 扩展](#3-配置-cc-扩展)
      - [4. 配置 CMake Tools](#4-配置-cmake-tools)
      - [5. 配置调试（launch.json）](#5-配置调试launchjson)
      - [6. 配置构建任务（tasks.json）](#6-配置构建任务tasksjson)
      - [7. VSCode 使用技巧](#7-vscode-使用技巧)
    - [2.1.2 零基础入门：如何开始写第一份代码](#212-零基础入门如何开始写第一份代码)
      - [步骤 1：创建项目目录](#步骤-1创建项目目录)
      - [步骤 2：创建项目基本结构](#步骤-2创建项目基本结构)
      - [步骤 3：编写第一份代码](#步骤-3编写第一份代码)
      - [步骤 4：配置 CMake（首次配置）](#步骤-4配置-cmake首次配置)
      - [步骤 5：编译项目](#步骤-5编译项目)
      - [步骤 6：运行程序](#步骤-6运行程序)
      - [步骤 7：调试代码](#步骤-7调试代码)
      - [步骤 8：修改代码并重新编译](#步骤-8修改代码并重新编译)
      - [步骤 9：常见问题排查](#步骤-9常见问题排查)
    - [2.2 项目结构设计](#22-项目结构设计)
    - [2.3 添加加密功能（在基础框架之上）](#23-添加加密功能在基础框架之上)
      - [步骤 1：实现命令解析器](#步骤-1实现命令解析器)
      - [步骤 3：实现第一个加密算法（凯撒密码）](#步骤-3实现第一个加密算法凯撒密码)
      - [步骤 4：集成加密功能到主程序](#步骤-4集成加密功能到主程序)
      - [步骤 5：使用 Makefile 简化编译](#步骤-5使用-makefile-简化编译)
    - [2.4 第二阶段：添加更多加密算法](#24-第二阶段添加更多加密算法)
    - [2.5 第三阶段：完善功能](#25-第三阶段完善功能)
      - [文件操作](#文件操作)
      - [配置管理](#配置管理)
      - [命令历史](#命令历史)
    - [2.6 跨平台编译技巧](#26-跨平台编译技巧)
      - [处理不同操作系统的差异](#处理不同操作系统的差异)
      - [使用 CMake（推荐，比 Makefile 更跨平台）](#使用-cmake推荐比-makefile-更跨平台)
  - [三、开发过程中可能遇到的问题及解决方案](#三开发过程中可能遇到的问题及解决方案)
    - [3.1 编译和链接问题](#31-编译和链接问题)
      - [问题 1：找不到头文件](#问题-1找不到头文件)
      - [问题 2：链接错误](#问题-2链接错误)
      - [问题 3：Windows 和 Linux 换行符不一致](#问题-3windows-和-linux-换行符不一致)
    - [3.2 用户使用问题](#32-用户使用问题)
      - [问题 1：用户需要安装编译环境吗？](#问题-1用户需要安装编译环境吗)
      - [问题 2：如何让用户方便地添加到环境变量？](#问题-2如何让用户方便地添加到环境变量)
      - [问题 3：跨平台兼容性问题](#问题-3跨平台兼容性问题)
    - [3.3 依赖管理问题](#33-依赖管理问题)
      - [问题：如何管理第三方库？](#问题如何管理第三方库)
    - [3.4 调试技巧](#34-调试技巧)
      - [使用 VSCode 调试（推荐）](#使用-vscode-调试推荐)
      - [使用命令行 GDB（Linux/macOS，进阶）](#使用命令行-gdblinuxmacos进阶)
      - [打印调试（简单但有效）](#打印调试简单但有效)
    - [3.5 性能优化](#35-性能优化)
      - [第一版不需要过度优化](#第一版不需要过度优化)
      - [基本优化原则](#基本优化原则)
  - [四、开发流程建议](#四开发流程建议)
    - [4.1 迭代式开发](#41-迭代式开发)
    - [4.2 版本控制（Git）](#42-版本控制git)
    - [4.3 测试建议](#43-测试建议)
      - [手动测试](#手动测试)
      - [自动化测试（进阶）](#自动化测试进阶)
  - [五、示例配置文件](#五示例配置文件)
    - [5.1 字符映射表（mapping.json）](#51-字符映射表mappingjson)
  - [六、常见问题（FAQ）](#六常见问题faq)
    - [Q1: 我应该用 C 还是 C++？](#q1-我应该用-c-还是-c)
    - [Q2: 需要使用面向对象编程吗？](#q2-需要使用面向对象编程吗)
    - [Q3: 如何确保代码质量？](#q3-如何确保代码质量)
    - [Q4: 项目可以用多久完成？](#q4-项目可以用多久完成)
    - [Q5: 学习资源推荐？](#q5-学习资源推荐)
  - [七、下一步行动](#七下一步行动)
  - [八、附录](#八附录)
    - [8.1 推荐的代码风格](#81-推荐的代码风格)
    - [8.2 有用的命令行参数解析](#82-有用的命令行参数解析)
    - [8.3 README 模板](#83-readme-模板)
  - [总结](#总结)
# CipherX CLI 工具开发指南
